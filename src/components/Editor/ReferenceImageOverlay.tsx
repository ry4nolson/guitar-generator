...
function ReferenceImage({ overlay }: { overlay: ReferenceOverlayRuntime }) {
  ...
  const sx = settings.flipH ? -1 : 1;
  const sy = settings.flipV ? -1 : 1;

  return (
    <g
      transform={`translate(${cx},${cy}) rotate(${rotation}) scale(${sx},${sy}) translate(${-width / 2},${-height / 2})`}
    >
      <image
        href={imageUrl}
        x={0}
...
/**
 * Click targets for visible reference images. Mounted above body/neck fills so
 * a click on the photo selects it in the sidebar; hardware/anchors/feature
 * strokes stay above and still win when overlapping.
 */
export function ReferenceOverlayHitTargets() {
  const { overlays, setActiveId } = useReferenceOverlayContext();
  const select = useDesignStore((s) => s.select);
  const visible = overlays.filter((o) => o.imageUrl && o.naturalSize && o.settings.visible);
  if (visible.length === 0) return null;

  return (
    <g id="reference-overlay-hits" data-reference-hits="true">
      {visible.map((overlay) => {
        const { width, height, cx, cy, rotation } = referenceImageLayout(
          overlay.settings,
          overlay.naturalSize!,
        );
        return (
          <g key={overlay.id} transform={`translate(${cx},${cy}) rotate(${rotation})`}>
            <rect
              x={-width / 2}
              y={-height / 2}
              width={width}
              ...
              onPointerDown={(e) => {
                ...
                select({ kind: 'reference', id: overlay.id });
              }}
            >
              <title>Select reference image</title>
            </rect>
          </g>
        );
      })}
...
/**
 * Dashed frame + rotate handle for the selected/active unlocked overlay, drawn
 * above the design so the reference can be dragged and rotated.
 */
export function ReferenceOverlayManipulator({ stageRef }: { stageRef: React.RefObject<SVGGElement | null> }) {
  ...
  const selectedRefId = selected?.kind === 'reference' ? selected.id : null;
  const target =
    (selectedRefId
      ? overlays.find(
          (o) =>
            o.id === selectedRefId && o.imageUrl && o.naturalSize && o.settings.visible,
        )
      : null) ??
    (activeOverlay &&
  ...
  const locked = target.settings.locked;
  const isSelected = selected?.kind === 'reference' && selected.id === overlayId;

  const beginDrag = (mode: 'move' | 'rotate', e: React.PointerEvent) => {
    e.stopPropagation();
    e.preventDefault();
    setActiveId(overlayId);
    select({ kind: 'reference', id: overlayId });
    if (locked) return;
  ...
  // Interior selection is handled by ReferenceOverlayHitTargets (below hardware).
  return (
    <g id="reference-overlay-manipulator" transform={`translate(${cx},${cy}) rotate(${rotation})`}>
      ...
      <rect
        ...
        height={height}
        fill="none"
        stroke={isSelected ? '#ff5533' : '#4aa3ff'}
